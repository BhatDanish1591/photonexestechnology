...
const ShaderRevealTransition = ({
  ...
  useEffect(() => {
    // If previousKey was not set yet (initial client mount/hydration), just set it and skip transition
    if (previousKey.current === undefined || previousKey.current === null || previousKey.current === "") {
    ...
    if (!controller) {
      setRenderedChildren(children);
      setIsActive(false); // Fail-safe: reset state so page isn't blocked if WebGL fails
      onRest?.();
      return;
    }
  ...
  return (
    <div
      className={cn(
        "relative isolate overflow-hidden rounded-2xl text-foreground",
        className
      )}
      data-transitioning={isActive ? "true" : "false"}
    >
      <div className="relative z-0">
        {isActive ? renderedChildren : children}
      </div>
